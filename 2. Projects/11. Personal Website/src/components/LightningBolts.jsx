...
// Midpoint displacement — builds an array of [x,y] points for a jagged bolt
...
// Draw a bolt path with a glow layer + bright core
...
function createStrike(w, h) {
  ...
  // Branches off random points on the main bolt
  ...
  // Sub-branches off branches (large bolts only)
  const subBranches = [];
  if (large) {
  ...
  return {
    ...
    large,
    alpha: 1,
    fadeSpeed: 0.022 + Math.random() * 0.018,
  };
}
...
export default function LightningBolts() {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // Remove faded
      strikes.splice(0, strikes.length, ...strikes.filter((s) => s.alpha > 0));
